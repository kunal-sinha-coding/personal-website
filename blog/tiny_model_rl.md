...
The initial supervised gain was the low hanging fruit in this experiment. One epoch improved pass@1 by 6.7 percentage points, or about 23 percent relative to the 29.3 percent baseline. This quickly moved the model into a more competent region, but later supervised epochs showed diminishing returns and eventual overfitting as training loss continued to fall while held-out performance declined.

I therefore restarted from a single SFT epoch and applied GRPO. The best GRPO checkpoint reached 31/75, which is 41.3 percent pass@1. This is only 1.3 percentage points above the earlier 40 percent result from GRPO without SFT, and the ongoing run later fell below that peak. The supervised warm start changed how quickly the model reached reasonable performance, but it has not yet demonstrated a higher final ceiling.

The practical objective is now to use SFT until its held-out gains saturate, then test whether outcome-based reinforcement learning can improve beyond that point. The difficult part is not obtaining the initial improvement from 29.3 percent to 36 percent. The difficult part is converting the stronger starting policy into a durable gain beyond the approximately 40 percent level already reached without SFT. Continued SFT and SFT followed by GRPO should be compared under matched additional compute so that any gain can be attributed to the objective rather than to more optimization steps.
